...
int main()
{
	//only let one copy of program start up.
	HANDLE hMutex = CreateMutex(NULL, TRUE, L"wordle.exe"); // L prefix for wide string

	if (hMutex != NULL && GetLastError() == ERROR_ALREADY_EXISTS) {
		std::cout << "Another instance of this program is already running." << std::endl;
		CloseHandle(hMutex);
		return 1; // Exit if another instance exists
	}
	else {


		 
		Application::Run(gcnew wordle::frmWordle());
	}
	...
	if (hMutex > 0) {
		ReleaseMutex(hMutex); // Release the mutex when the program exits
		CloseHandle(hMutex);
	}
	return 0;
}
...
{	
	...
	return binarySearchRecursive(sortedArrayOfWords, input, first, last);
